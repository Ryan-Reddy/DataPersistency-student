...
FROM (SELECT DISTINCT cursist FROM inschrijvingen WHERE cursus = 'XML') AS xml
         JOIN
...
SELECT mnr
FROM medewerkers
WHERE mnr NOT IN (SELECT DISTINCT cursist
                  FROM inschrijvingen
                  WHERE cursus = 'JAV')
;
...
SELECT naam
FROM medewerkers
WHERE mnr IN (SELECT DISTINCT chef
              FROM medewerkers);
...
SELECT cursussen.code, begindatum
FROM uitvoeringen
         JOIN cursussen
...
SELECT DISTINCT cursus,
                ...
                (SELECT COUNT(cursist)
                 ...
                 WHERE uita.cursus = insch.cursus
                   AND uita.begindatum = insch.begindatum) AS aantal_inschrijvingen
